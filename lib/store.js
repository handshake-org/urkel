...
const META_SIZE = 4 + Pointer.getSize() + NodePointer.getSize() + 20;
...
class Store {
  constructor(fs, prefix, hash, bits) {
    ...
    this.internalSize = Internal.getSize(hash, bits);
    this.leafSize = Leaf.getSize(hash, bits);
    this.maxWrite = this.leafSize + 0xffff + this.internalSize + META_SIZE;
  ...
  async readNode(ptr) {
    const data = await this.read(ptr.index, ptr.pos, ptr.size(this));
    return this.decodeNode(data, ptr);
  }

  readNodeSync(ptr) {
    const data = this.readSync(ptr.index, ptr.pos, ptr.size(this));
    return this.decodeNode(data, ptr);
  }
...
class Meta {
  ...
  read(data, off, hash, key) {
    ...
    this.rootPtr = NodePointer.read(data, off);
