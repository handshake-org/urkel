...
class Store {
  constructor(fs, prefix, hash, bits) {
    ...
    this.maxWrite = this.leafSize + 0xffff + this.nodeSize;
  ...
  needsFlush() {
    return this.wb.written >= WRITE_BUFFER - this.maxWrite;
  }
  ...
  writeMeta(root) {
    ...
    assert(off - this.wb.written === META_SIZE);

    this.wb.written += META_SIZE;

    state.metaIndex = this.wb.index;
    state.metaPos = this.wb.pos - META_SIZE;
  ...
  parseMeta(data, off) {
    try {
      return Meta.read(data, off, this.key);
    } catch (e) {
      return null;
    }
  }
  ...
  async readMeta(index, pos) {
    ...
    return Meta.decode(data, this.key);
  }
...
class Meta {
  ...
  write(data, off, key) {
    ...
    key.copy(data, off + 4);
    writeU16(data, this.metaIndex, off + 20);
    writeU32(data, this.metaPos, off + 22);
    writeU16(data, this.rootIndex, off + 26);
    writeU32(data, rootPos, off + 28);

    const preimage = data.slice(off + 20, off + 32);
    const checksum = hashPerfect(preimage);

    checksum.copy(data, off + 32);

    assert(32 + checksum.length === META_SIZE);
  ...
  read(data, off, key) {
    ...
    const k = data.slice(off + 4, off + 20);

    if (!k.equals(key))
      throw new Error('Invalid metadata key.');

    const preimage = data.slice(off + 20, off + 32);
    const checksum = data.slice(off + 32, off + 32 + 12);
    const expect = hashPerfect(preimage);

    if (!checksum.equals(expect))
      throw new Error('Invalid metadata checksum.');

    this.metaIndex = readU16(data, off + 20);
    this.metaPos = readU32(data, off + 22);
    this.rootIndex = readU16(data, off + 26);
    this.rootPos = readU32(data, off + 28);
  ...
  static read(data, off, key) {
    return new this().read(data, off, key);
  }

  static decode(data, key) {
    return new this().decode(data, key);
  }
