...
class Store {
  ...
  async commit(root) {
    ...
    if (!node.isNull())
      this.rootCache.set(node.data, node);
  }
  ...
  async readRoot(index, pos) {
    ...
    if (!this.rootCache.has(hash))
      this.rootCache.set(hash, node.toHash(this.hash));
  ...
  async getHistory(rootHash) {
    ...
    const cached = this.rootCache.get(rootHash);
