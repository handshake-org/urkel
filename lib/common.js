...
const zeroHashes = [];
const emptyRoots = new WeakMap();

for (let i = 0; i < 64; i++)
  zeroHashes.push(null);

exports.zeroHash = function zeroHash(size) {
  if (!zeroHashes[size])
    zeroHashes[size] = Buffer.alloc(size, 0x00);
  return zeroHashes[size];
};

exports.emptyRoot = function emptyRoot(hash) {
  const key = hash;
  const cache = emptyRoots.get(key);

  if (cache)
    return cache;

  const root = hash.digest(zeroByte);

  emptyRoots.set(key, root);

  return root;
};

exports.hashSize = function hashSize(hash) {
  const root = exports.emptyRoot(hash);
  return root.length;
};

exports.ensureHash = function ensureHash(hash) {
  if (hash.size != null)
    return hash;

  hash.id = hash.name.toLowerCase();
  hash.size = exports.hashSize(hash);
  hash.bits = hash.size * 8;
  hash.zero = exports.zeroHash(hash.size);
