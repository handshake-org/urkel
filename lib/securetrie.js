...
class SecureTrie {
  ...
  constructor(hash, db, limit) {
    ...
    this.trie = new Trie(hash, db, limit);
    this.hash = this.trie.hash;
  ...
  snapshot(root) {
    const {hash, db, cacheLimit} = this.trie;
    const st = new this.constructor(hash, db, cacheLimit);
  ...
  secKey(key) {
    ...
    assert(key.length === this.hash.size, 'Bad key size.');
    const buf = Buffer.allocUnsafe(11 + this.hash.size);
