...
/*
This should be done in a configuration file
*/
...
console.log("Meta size", META_SIZE)
  ...
  */
  ...
  async readdir() {
    ...
    for (const name of names) {
      ...
      if (index === -1 || index === 0 || index > MAX_FILES )
        continue;
  ...
  async closeFile(index) {
    ...
    this.checkFileOpen();
    const file = this.files.get(index);
//TODO what is difference between undefined and null Why use one and not the other
    if (!file || file.reads > 0)
      return undefined;
...
//TODO what is difference between undefined and null Why use one and not the other
      if (index === this.index || file.reads < 0)
        continue;
...
// TODO Nooooooo
  ...
  async retrieve(node) {
    ...
    assert(node.vptr);
    //TODO need the key offset Next :) TF also the read statement is dangerous
...
class Meta {
  ...
  write(data, off, hash, key) {
    ...
    assert(key.length === KEY_SIZE); //TODO
...
/**
 * Write Buffer
 * offset, int, Start position in file.
 * index, int, current file index
 * start, int, where the current file starts in memory
 * written, int, current buffer position
 */
