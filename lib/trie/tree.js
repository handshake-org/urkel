...
class Tree {
  ...
  constructor(hash, bits, prefix) {
    ...
    assert((bits >>> 0) === bits); //wtf
    assert(bits > 0 && (bits & 7) === 0); //no bueno
  ...
  async _insert(node, key, value, depth) {
    switch (node.type()) {
      ...
      case LEAF: {
        ...
        const bits = node.bits.collision(key, depth);
        ...
        let bit = hasBit(key, depth);
        let next = Internal.from(leaf, node, bit);

        for (let i = 0; i < bits; i++) {
          depth -= 1;
          bit = hasBit(key, depth);
          next = Internal.from(next, NIL, bit);
        }
  ...
  async _remove(node, side, key, depth) {
    switch (node.type()) {
      case NULL: {
        // Empty (sub)tree.
        return [null, 0];
      ...
      case INTERNAL: {
        ...
        const [z, state] = await this._remove(x, y, key, depth + 1);
        ...
        return [Internal.from(z, y, bit), 0];
      }
      ...
      case LEAF: {
        ...
        const sibling = await this.resolve(side);
      ...
      case HASH: {
        const rn = await this.resolve(node);
        return this._remove(rn, side, key, depth);
      }
