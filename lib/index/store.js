...
class Meta {
  ...
  write(data, off) {
    ...
    writeU32(data, this.rootPos, off + 8, true);
    data[off + 8] = this.rootLeaf ? 1 : 0;

    checksum.copy(data, off + 9, 0, 8);
  ...
  read(data, off) {
    ...
    this.rootIndex = readU32(data, off + 0);
    this.rootPos = readU32(data, off + 4);
    this.rootLeaf = (data[off + 8] & 1) !== 0;

    const checksum = data.slice(off + 9, off + 9 + 8);
...
function hash(index, pos, leaf) {
  const out = Buffer.allocUnsafe(8);

  index |= (leaf << 31) >>> 0;
  index >>>= 0;

  writeU32(out, phf(index), 0);
  writeU32(out, phf(pos), 4);
