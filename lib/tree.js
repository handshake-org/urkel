...
class Tree {
  ...
  async resolveValue(node) {
    return this.store.resolveValue(node);
  }

  async _get(root, key) {
    let node = await this.resolve(root);
    let depth = 0;
  ...
  async _remove(root, key) {
    ...
    for (;;) {
      ...
      if (node.isLeaf()) {
        ...
        // Sibling.
        const sibling = nodes.pop();
        depth -= 1;

        // Shrink the subtree if we're a leaf.
        if (sibling.hasLeaf()) {
          const other = await this.resolve(sibling);

          // Sanity check (last comparison should have been different).
          assert(hasBit(key, depth) !== hasBit(other.key, depth));

          while (depth > 0) {
            const side = nodes[depth - 1];

            if (hasBit(key, depth - 1) !== hasBit(other.key, depth - 1))
              break;
  ...
  async _compact(node, store) {
    ...
    switch (node.type()) {
      ...
      case LEAF: {
        node.index = 0;
        node.pos = 0;
        node.value = await this.resolveValue(node);
...
class Transaction extends Snapshot {
  ...
  async getRoot() {
    if (this.root.isHash())
      this.root = await this.tree.resolve(this.root);
    return this.root;
  }
  ...
  async next() {
    for (;;) {
      ...
      if (this.read && !this.node.value) {
        this.value = await this.tree.resolveValue(this.node);
