...
class Merklix {
  ...
  constructor(hash, db, limit, pruneMode) {
    if (limit == null)
      limit = 500000;
    ...
    this.pruneList = new PruneList(this.hash.size);
  }
  ...
  async get(key) {
    return await this._get(this.root, key);
  }
  ...
  async getPruneList(root) {
    ...
    return [key, PruneList.decode(data, this.hash.size)];
  }
  ...
  commit(batch, enc) {
    ...
    switch (this.pruneMode) {
      ...
      case 1:
        ...
        break;
      case 2:
        this.pruneList.save(batch, this.root);
        break;
      default:
        throw new AssertionError('Unknown prune mode.');
    }
  ...
  snapshot(root) {
    ...
    const {hash, db} = this;
    const tree = new this.constructor(hash, db);
  ...
  verify(root, key, proof) {
    return proofs.verify(this.hash, root, key, proof);
  }
...
proofs.verify = function verify(hash, root, key, proof) {
  const nodes = proof.nodes;

  if (nodes.length === 0)
    return [PROOF_EARLY_END, null];

  if (nodes.length > hash.bits)
...
class Proof {
  ...
  zeroHash() {
    return Buffer.alloc(this.hashSize(), 0x00);
  }
  ...
  encode() {
    ...
    const zeroHash = this.zeroHash();
    const size = this.getSize(zeroHash);
    ...
    assert(this.nodes.length > 0);
    assert(this.nodes.length <= hashSize * 8);
  ...
  decode(data, hashSize) {
    ...
    if (field & (1 << 15)) {
      assert(pos + hashSize <= data.length);
      this.key = data.slice(pos, pos + hashSize);
      pos += hashSize;
    }
  ...
  static decode(data, hashSize) {
    return new this().decode(data, hashSize);
  }
}
