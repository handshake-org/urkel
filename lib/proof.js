...
exports.prove = async function prove(trie, root, key) {
  const nodes = [];
  ...
  const hasher = new Hasher(trie.hash, 0, 0);
  const proof = [];
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    if (hn.isHash() || i === 0)
      proof.push(nn.encode(trie.hash));
  }
...
exports.verify = function verify(hash, root, key, proof) {
  ...
  for (let i = 0; i < proof.length; i++) {
    ...
    if (!hash.digest(raw).equals(h))
      return [PROOF_HASH_MISMATCH, null];

    let n;
    try {
      n = decodeNode(raw, hash);
    } catch (e) {
