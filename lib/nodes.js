...
class Internal extends Node {
  ...
  write(data, off, hash, bits) {
    ...
    off = writeU16(data, left.index * 2 + 1, off);
...
class Leaf extends Node {
  ...
  write(data, off, hash, bits) {
    off = writeU16(data, this.vindex * 2, off);
...
class Hash extends Node {
  ...
  async resolve(store) {
    const node = await store.readNode(this.index, this.pos, this.leaf);
    node.data = this.data;
    return node;
  }

  resolveSync(store) {
    const node = store.readNodeSync(this.index, this.pos, this.leaf);
    node.data = this.data;
    return node;
  }
